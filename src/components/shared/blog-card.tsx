...
export function BlogCard({ p }: { p: BlogPost }) {
  return (
    <article className="card-base">
      <div className="thumb-pat relative grid h-[150px] place-items-center" style={{ background: `linear-gradient(135deg, ${p.color}, #0D1B4B)` }}>
        <span className="relative rounded-md bg-white/15 px-3 py-1 text-[12px] font-bold uppercase tracking-wide text-white">{p.category}</span>
      </div>
